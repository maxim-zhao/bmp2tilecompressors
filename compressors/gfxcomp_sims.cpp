...
std::vector<uint8_t> compress(const std::vector<uint8_t>& data)
{
    ...
    for (int position = lenData - 1; position >= 0; --position)
    {
        ...
        for (int rleMatchLength = 1; rleMatchLength < 5; ++rleMatchLength)
        {
            ...
            if (lenData - position < rleMatchLength * 2)
            {
                continue;
            }
            // Grab the sequence
            const auto& rleSequence = std::ranges::view<uint8_t>(data.begin() + position, data.begin() + position + rleMatchLength);
            auto rleCount = 1;
            // Check for matches
            for (int matchPosition = position + rleMatchLength; 
                matchPosition < std::min(lenData, position + rleMatchLength * 2047); 
