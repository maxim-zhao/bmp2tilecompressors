...
void loadfile(/*string sourcefilename*/const unsigned char* source) {
  //FILE *file;
  ...
  //if (!fread(d,length,1,file)) { cout << "Filesize error" << endl; exit(1); }
    // Added: copy data to d
    memcpy_s(d, length+1, source, length);
  d[length]=0;
  //fclose(file);
}
...
int pletter_main(/*int argc, char *argv[]*/const unsigned char* source, int len, unsigned char* dest, int destLen) {
//  if (argc==1) cout << endl;
...
//  if (!destfilename[0]) destfilename=sourcefilename+".plet5";
  ...
  loadfile(/*sourcefilename*/source);
...
//    cout << ".";
  ...
  save(p[minbl],minbl);
    ...
    memcpy_s(dest, destLen, s.buf, s.dp);
