...
const int MAX_RUN_SIZE = 0x7f;
const int RLE_MASK = 0x00;
const int RAW_MASK = 0x80;
...
void writeRaw(std::vector<uint8_t>& buffer, std::vector<uint8_t>::const_iterator begin, const std::vector<uint8_t>::const_iterator end)
{
	while (begin < end)
	{
...
int compress(const uint8_t* pSource, const uint32_t sourceLength, uint8_t* pDestination, const uint32_t destinationLength, const uint32_t interleaving)
{
	...
	{
		const int runLength = getRunLength(it, bufSource.cend());
		int runLengthNeeded = 3; // normally need a run of 3 to be worth breaking a raw block
		if (it == bufSource.cbegin() || it + runLength == bufSource.cend()) --runLengthNeeded; // but at the beginning or end, 2 is enough
